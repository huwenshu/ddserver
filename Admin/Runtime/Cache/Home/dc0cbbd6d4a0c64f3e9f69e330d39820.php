...
<body>
 ...
 foreach ($reqList as $key => $value) { $temp = "<li name='moneyitem' class='mui-table-view-cell'><p>停车场：".$value['park']." 申请人：".$value['name']."</p>
                            ...
                            <p>电话：".$value['telephone']." 申请时间：".$value['createtime']."</p>"; if($value['state'] == 0){ $url = U('moneyreqDone')."/rid/".$value['id']; $temp = $temp."<a href='".$url."'>完成提现</a>"; } else{ $temp = $temp."<span>已提现</span>"; } echo $temp; } ?>
        </ul>
