   ...
   <!-- /边栏 -->
    ...
    <div id="indexMain" class="index-main">
        ...
        <ul class="mui-table-view">
            <?php
 foreach ($reqList as $key => $value) { $temp = "<li name='giftitem' class='mui-table-view-cell'><p>停车场：".$value['park']." 申请人：".$value['adminName']."</p>
            <p>礼品：".$value['giftName']." 花费积分：".$value['score']."</p>
            <p>收件人：".$value['name']." 地址：".$value['address']."</p>
            <p>电话：".$value['telephone']." 申请时间：".$value['createtime']."</p>"; if($value['state'] == 0){ $url = U('giftreqDone')."/gid/".$value['id']; $temp = $temp."<a href='".$url."'>完成兑换</a></li>"; } else{ $temp = $temp."<span>已兑换</span></li>"; } echo $temp; } ?>
        </ul>
