 ...
 * 自定义的APP支付接口
 */
class AppApi_pub extends Wxpay_client_pub
{
    ...
    function createXml()
    {
        try
        {
            ...
            $this->parameters["appid"] = WxPayConf_pub::M_APPID;//APP支付的公众账号ID
            $this->parameters["mch_id"] = WxPayConf_pub::M_MCHID;//APP支付的商户号
            $this->parameters["spbill_create_ip"] = $_SERVER['REMOTE_ADDR'];//终端ip
            $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
            $this->parameters["sign"] = $this->getSign($this->parameters);//签名
            return  $this->arrayToXml($this->parameters);
    ...
    public function getParameters()
    {
        //封装返回数据
        $appPay = array();
        $appPay['appid'] =  WxPayConf_pub::M_APPID;//APP支付的APPID
        $appPay['noncestr'] =  $this->createNoncestr();
